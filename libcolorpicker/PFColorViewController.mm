...
@interface PFColorViewController () <PFColorPickerDelegate> {
...
- (id)initForContentSize:(CGSize)size {
    if ([NSClassFromString(@"PSViewController") instancesRespondToSelector:@selector(initForContentSize:)])
...
- (void)loadCustomViews {
    ...
    Class viewClass;
    if (NSClassFromString(@"UIBackdropView"))
        viewClass = NSClassFromString(@"UIBackdropView");
    else
...
- (void)chooseHexColor {
    ...
    [alertController addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
        textField.text = [UIColor hexFromColor:self->_pushedView.backgroundColor];
    }];
    ...
    [alertController addAction:[UIAlertAction actionWithTitle:@"Set" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        if ([userHexString hasPrefix:@"#"] && [UIColor PF_colorWithHex:userHexString])
            [self pickedColor:[UIColor PF_colorWithHex:userHexString]];
    }]];
    ...
    [alertController addAction:[UIAlertAction actionWithTitle:@"Copy" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [[UIPasteboard generalPasteboard] setString:[UIColor hexFromColor:self->_pushedView.backgroundColor]];
    }]];
...
+ (UIImage *)thumbImageWithColor:(UIColor *)color letter:(unichar)letter {
    ...
    // FIXME: use core text / fix deprecated warnings
